      ...
      >
                ...
                <span class="d-flex justify-space-between block" v-if="head.value != 'data-table-select'">
                  ...
                  <div v-if="head.filterable">
                    <v-btn x-small icon class="filter-action" @click="sortTable(head)">
                      <v-icon> {{head.sortType == null ? 'mdi-sort' : (head.sortType == 1 ? 'mdi-sort-ascending' : 'mdi-sort-descending')}}</v-icon>
                    </v-btn>
                  </div>
                </span>
                <div v-else class="v-data-table__checkbox v-simple-checkbox" @click="toggleMultipleItems">
                  <div class="v-input--selection-controls__input">
                    <i aria-hidden="true" :class="['v-icon notranslate mdi theme--light', isAllSelected ? 'mdi-checkbox-marked' : 'mdi-checkbox-blank-outline']"></i>
        ...
        <template v-slot:body.prepend="{item}" v-if="loaded == 2">
          <v-scroll-y-transition>
            <tr class="filter-row" v-show="showFilter || (isFiltered && !filterStatusChanged)" transition="fade-transition">
              <td class="py-2"></td>
